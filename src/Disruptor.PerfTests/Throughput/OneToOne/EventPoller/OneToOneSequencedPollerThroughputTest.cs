...
namespace Disruptor.PerfTests.Throughput.OneToOne.EventPoller;
...
public class OneToOneSequencedPollerThroughputTest : IThroughputTest
{
    ...
    {
        private readonly EventPoller<PerfEvent> _poller;
        private readonly Disruptor.EventPoller.Handler<PerfEvent> _eventHandler;
        ...
        public void Run()
        {
            try
            {
                ...
                {
                    _started.Set();
                    if (Disruptor.EventPoller.PollState.Processing != _poller.Poll(_eventHandler))
                    {
                        Thread.Sleep(0);
                    }
                }
